...
O sistema é dividido em duas aplicações:
 - Backend em `src/api` (Node.js/Express)
 - Frontend em `src/frontend-app` (Svelte + Vite, tema escuro unificado)
...
Com o projeto clonado (sem pastas `node_modules`) ir na pasta `src/api` e instalar as dependências:

```bash
cd src/api
cp .env.example .env   # edite JWT_SECRET antes de rodar
npm install
npm run dev
```
...
Com o projeto clonado (sem pastas `node_modules`) ir na pasta `src/frontend-app` e instalar as dependências:
